...
* @date 16/05/2018
* @compile "gcc LLCARPIP.c -o LLCARPIP"
* @run "./LLCARPIP "
*/
#include <stdlib.h>
#include <stdio.h>
...
bool Analizer(byte T[], int i){
	...
	//#########################################	PROTOCOLO IP 	#####################################################################
		...
		Suma=0;
		sprintf(resultado+strlen(resultado),"\n##################Protocolo IP ####################\n Version : %hhu",T[14]>>4);
		byte IHL=(T[14]&0b1111)*4;
		sprintf(resultado+strlen(resultado),"\n Internet Header Length : %d bytes",IHL);
		sprintf(resultado+strlen(resultado),"\n Tipo de Servicio :");
		...
		TamanoTotal=TamanoTotal|T[16]<<8;
		TamanoTotal|=T[17];

		sprintf(resultado+strlen(resultado),"\n Tamaño total : %u",TamanoTotal);
		sint Identificacion=0;

		Identificacion=Identificacion|T[18]<<8;
		Identificacion|=T[19];
		...
		for (byte i = 14; i < IHL+14; i+=2)
			{
				if(i==24|i==25){

				}
				else if ((IHL+14)<(i+1))//Si ya estas en el limite checame si eres el ultimo
				{
					//Envias bit mas significativo y bit menos significativo
					Checksum(0,T[i]);
				}
				else
				{
		...
		sprintf(resultado+strlen(resultado),"\n IP Origen: %hhu.%hhu.%hhu.%hhu",T[26],T[27],T[28],T[29]);

		sprintf(resultado+strlen(resultado),"\n IP Destino: %hhu.%hhu.%hhu.%hhu",T[30],T[31],T[32],T[33]);

		sprintf(resultado+strlen(resultado),"\n Opciones: ");
		i=34;
		
		while(i<IHL+14){
			sprintf(resultado+strlen(resultado)," %hhu ",T[i] );
			i++;
		}
...
//},
{
0x13, 0x42, 0x18, 0x63, 0x96, 0x12, 0x13, 0x42, 0x18,
0x96, 0x35, 0x72, 0x08, 0x00, 0x46, 0x10, 0x80, 0x42, //0x10 0x80 0x42
0x04, 0x55, 0x34, 0x11, 0x80, 0x11, 0x6b, 0xf0, 0x94,
0xcc, 0x39, 0xcb, 0x94, 0xcc, 0x67, 0x02, 0xaa, 0xbb,
0xcc, 0xdd, 0x04, 0x0c, 0x00, 0x35, 0x00, 0x2e, 0x85,
0x7c, 0xe2, 0x1a, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x03, 0x77, 0x77, 0x77, 0x03,
0x69, 0x73, 0x63, 0x05, 0x65, 0x73, 0x63, 0x6f, 0x6d,
0x03, 0x69, 0x70, 0x6e, 0x02, 0x6d, 0x78, 0x00, 0x00,
0x1c, 0x00, 0xa1
},
